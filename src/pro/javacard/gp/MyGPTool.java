...
import pro.javacard.gp.SEAccessControl.AidRefDo;
import pro.javacard.gp.SEAccessControl.ApduArDo;
import pro.javacard.gp.SEAccessControl.ArDo;
import pro.javacard.gp.SEAccessControl.EventAccessRules;
import pro.javacard.gp.SEAccessControl.HashRefDo;
import pro.javacard.gp.SEAccessControl.NfcArDo;
import pro.javacard.gp.SEAccessControl.RefArDo;
import pro.javacard.gp.SEAccessControl.RefDo;


public final class MyGPTool {
					...
					if (needsAuthentication(args)) {
						...
						if (args.has(OPT_ACR_ADD)){
							...
							if (args.has(OPT_APPLET)) {
								...
								aidRefDo = new AidRefDo(aid.getBytes());
							} else
								aidRefDo = new AidRefDo(new byte[0]);

							if (args.has(OPT_ACR_CERT_HASH)) {
								byte[] hash = (byte[]) args.valueOf(OPT_ACR_CERT_HASH);
								if (hash.length != 20)
									throw new CardException("certificate hash must be 20 bytes");
								hashRefDo = new HashRefDo(hash);
							} else
								hashRefDo = new HashRefDo(new byte[0]);

							RefDo refDo = new RefDo(aidRefDo, hashRefDo);

							if (!args.has(OPT_ACR_RULE))
								throw new CardException("Must specify an access rule with -" + OPT_ACR_RULE  + " (00, 01 or an apdu filter)");

							byte[] rule = (byte[]) args.valueOf(OPT_ACR_RULE);
							if (rule[0] == EventAccessRules.ALWAYS.getValue()) {
								arDo = new ArDo(
										new ApduArDo(EventAccessRules.ALWAYS, new byte[]{}),
										new NfcArDo(EventAccessRules.ALWAYS));
							} else if (rule[0] == EventAccessRules.NEVER.getValue()) {
								arDo = new ArDo(
										new ApduArDo(EventAccessRules.NEVER, new byte[]{}),
										new NfcArDo(EventAccessRules.NEVER));
							} else
								throw new CardException("Not Supported ARA rule: " + HexUtils.bin2hex(rule));


							RefArDo refArDo = new SEAccessControl.RefArDo(refDo, arDo);
							SEAccessControl.StoreArDo storeArDo = new SEAccessControl.StoreArDo(refArDo);
							SEAccessControlUtility.acrStore(gp, storeArDo);
						}
						...
						if (args.has(OPT_ACR_DELETE)){
							...
							SEAccessControl.RefArDo refArDo = new SEAccessControl.RefArDo(refDo, arDo);
							SEAccessControl.TLV request = new SEAccessControl.DeleteArDo(refArDo);
